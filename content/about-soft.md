...
3. **代码重用**：重用函数、类和软构件（C函数库、MFC类库、Java SDK、ROS节点）；重用Github开源代码
4. **结对编程**：团队开发、合作开发、群体开发
...
```
需求分析(What) → 设计(How) → 编码实现 → 测试 → 部署运行 → 维护
```
...
将**系统的、规范的、可量化的**方法应用于软件的开发、运行和维护的过程。
...
以文档为中心的**重型方法**：大量时间写文档和评审，难应对需求变化，较长时间才能得到可运行软件
...
**MoSCoW优先级法：**
- **M**ust have（必须有的）：核心功能，没有则系统不可用
- **S**hould have（应该有的）：重要功能，短期可暂缺
- **C**ould have（可以有的）：锦上添花，有则更好
- **W**on't have（不需要的）：本次不开发，以后考虑
...
| 顺序图 | 描述对象间消息传递的时间顺序 |
| 通信图 | 描述对象间消息传递的结构关系 |
| 类图（分析层次） | 边界类(<<boundary>>)、控制类(<<control>>)、实体类(<<entity>>) |
| 状态图 | 描述对象的状态变迁 |
...
1. **软件体系结构设计**：全局宏观视角，高层抽象，模块为"黑盒子"
2. **用户界面设计**：软件对外展示和人机交互
3. **软件详细设计**：细化精化，包括数据设计、接口设计、类设计、算法设计
...
```
用户动作 → Controller(控制器) → Model(模型) → View(视图) → 用户
```

### 九、体系结构设计过程

1. 设计初步体系结构（辨识关键需求 → 选择合适风格 → 初创架构）
2. 重用开源软件及已有软件资产
3. 精化体系结构（确定基础设施 → 确立子系统/构件/类 → 接口协作）
...
1. **用户分析**：理解用户特征、使用场景和操作习惯
2. **任务分析**：分析用户需要完成的操作任务
3. **界面原型设计**：绘制线框图和交互原型
4. **界面实现**：编写前端代码实现界面
5. **可用性评估**：通过用户测试检验界面质量
...
1. 确定可见范围（public/protected/private，尽量缩小可见范围）
2. 精化类间关系（继承 > 组合 > 聚合 > 关联 > 依赖）
3. 构造状态图和活动图
4. 精化属性和方法
5. 评审和优化
...
1. **持续变化法则**：不持续修改会越来越不实用
2. **增加复杂性法则**：除非采取措施降低复杂性，否则越来越复杂
3. **自我调节法则**：演化过程自我可调节
4. **组织稳定性守恒法则**：新版本平均额外工作量相同
5. **熟悉度守恒法则**：平均增量增长相同
6. **功能持续增长法则**：功能必须持续增加，否则用户满意度降低
7. **质量衰减法则**：代码变更导致质量逐渐下降
8. **反馈系统法则**：多回路活动和多层次反馈
